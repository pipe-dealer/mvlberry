...
func Newchannel(f_id int) *ChatChannel {
...
func (chatchannel *ChatChannel) Start() {
	for {
		select {
		...
		case client := <-chatchannel.Joined:
			...
			sysMsg := Message{
				MessageType: 1,
				Msg:         []byte("User has joined"),
			}
			fmt.Println("A user has joined channel ", chatchannel.Id)
		...
		case client := <-chatchannel.Left:
			...
			sysMsg := Message{
				MessageType: 1,
				Msg:         []byte("User has left"),
			}
			fmt.Println("A user has left channel ", chatchannel.Id)
